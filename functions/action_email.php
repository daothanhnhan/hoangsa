...
class action_email 
{
        ...
        //         $nFrom = "NÂU STORE";    //mail duoc gui tu dau, thuong de ten cong ty ban
        ...
        public function lien_he1 () {
            global $conn_vn;
                if (isset($_POST['lien_he'])) {
                        ...
                        $address = '';
                        $note = $_POST['note'];
                        $chondichvu = $_POST['chondichvu'];
                        $sql = "INSERT INTO lien_he (name, email, phone, address, comment, chondichvu) VALUES ('$name','$email','$phone','$address','$note', '$chondichvu')";
                        $result = mysqli_query($conn_vn, $sql);
                        // $str = $this->form($name, $phone, $email, $note);
                        //echo $str;die;
                        // $this->email_send($email, "Thông Tin Khách Hàng Từ V-ONION", $str);
